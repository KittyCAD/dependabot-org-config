    ...
    for repo in repos.iter().progress() {
        ...
        if !updates.is_empty() {
            ...
            if args.verbose {
                println!(
                    "{}",
                    yaml_peg::serde::to_string(&config).context("failed to print yaml")?
                );
            }
...
async fn create_pr(
    ...
    let content = yaml_peg::serde::to_string(&config)?;
...
async fn get_dependabot_yml(
    ...
    let mut config_vec = yaml_peg::serde::from_str::<DependabotConfig>(&text)?;
    match config_vec.as_mut_slice() {
        [config] => Ok(Some((config.clone(), content.sha.clone()))),
        _ => anyhow::bail!(
            "Expected exactly one DependabotConfig, found {}",
            config_vec.len()
        ),
    }
}
