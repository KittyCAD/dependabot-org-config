    ...
    for repo in repos.iter().progress() {
        ...
        if !updates.is_empty() {
            ...
            if args.verbose {
                println!(
                    "{}",
                    serde_norway::to_string(&config).context("failed to print yaml")?
                );
            }
...
async fn create_pr(
    ...
    let content = serde_norway::to_string(&config)?;
...
async fn get_dependabot_yml(
    ...
    let config = serde_norway::from_str::<DependabotConfig>(&text)?;
